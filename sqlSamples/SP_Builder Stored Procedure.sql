CREATE PROCEDURE [Development].[SPBuilder]
(
   @TableName  SYSNAME
 , @SchemaName SYSNAME = NULL
 , @Create     BIT     = 1 /* Create the porocedures automatically.*/

)
AS
  BEGIN
    SET NOCOUNT ON;
    /* If no schema is provided, assumes it is dbo*/
...
*/
    ...
    DECLARE 
     ...
     , @Insert         NVARCHAR(MAX)
     , @InsertDrop     NVARCHAR(MAX)
     , @identityColumn NVARCHAR(300)
     , @where          NVARCHAR(MAX)
     , @ParamsDelete   NVARCHAR(MAX)
     , @Update         NVARCHAR(MAX)
     , @UpdateDrop     NVARCHAR(MAX)
     , @Delete         NVARCHAR(MAX)
     , @DeleteDrop     NVARCHAR(MAX);
...
*/
    ...
    IF @Create = 1
      ...
      END;
    ELSE 
    /* Returns the sentences as string*/
      BEGIN
        SELECT 
           @InsertDrop
           + @CrLf
